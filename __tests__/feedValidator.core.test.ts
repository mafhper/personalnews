  ...
  describe("validateFeed", () => {
    it("should validate a valid RSS feed", async () => {
      ...
      (global.fetch as any).mockResolvedValueOnce({
        ...
        headers: {
          get: (name: string) =>
            name === "content-type" ? "application/rss+xml" : null,
        },
      ...
      expect(result.route?.transport).toBe("client");
    });
    ...
    it("should validate a valid Atom feed", async () => {
      ...
      (global.fetch as any).mockResolvedValueOnce({
        ...
        headers: {
          get: (name: string) =>
            name === "content-type" ? "application/atom+xml" : null,
        },
    ...
    it("should handle HTTP errors", async () => {
      ...
      expect(result.diagnostics?.cause).toBe("not_found");
      expect(result.diagnostics?.action).toBeTruthy();
    });
    ...
    it("should handle network errors", async () => {
      ...
      const result = await feedValidator.validateFeed(
        "https://unreachable.com/rss.xml",
      );

      expect(result.isValid).toBe(false);
      expect(result.status).toBe("network_error");
      expect(result.error).toContain(
        "Network error occurred while fetching the feed",
      );
      expect(result.validationAttempts.length).toBeGreaterThan(0);
      expect(result.route?.routeName).toBeTruthy();
      expect(result.diagnostics?.action).toBeTruthy();
    });
    ...
    it("should handle invalid XML content", async () => {
      ...
      expect(result.diagnostics?.cause).toBe("parse_error");
    });
