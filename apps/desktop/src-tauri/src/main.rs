...
fn start_backend_sidecar(app: &AppHandle) -> Result<(), String> {
    ...
    log_desktop_event(
        app,
        &format!("event=start_requested base_url={base_url} db_path={backend_db_path}"),
    );
    ...
    *token_guard = Some(backend_auth_token);
    ...
    log_desktop_event(
        app,
        &format!("event=spawn_succeeded pid={pid} base_url={base_url}"),
    );
    let process_started_at = Instant::now();
    monitor_backend_process(app.clone(), pid, process_started_at, rx);
    monitor_backend_readiness(app.clone(), base_url, pid, process_started_at);
    Ok(())
}
            ...
            match event {
                CommandEvent::Stdout(line) => {
                    ...
                    if text.contains("PERSONALNEWS_BACKEND_READY") {
                        mark_backend_ready(&app, pid, process_started_at, "stdout_ready_seen");
                    }
                }
...
fn mark_backend_ready(app: &AppHandle, pid: u32, process_started_at: Instant, event_name: &str) {
    let state = app.state::<BackendSidecarState>();
    ...
    if let Some(base_url) = base_url {
        log_desktop_event(
            app,
            &format!("event={event_name} pid={pid} base_url={base_url} uptime_ms={uptime_ms}"),
        );
        emit_backend_status_changed(app);
...
fn monitor_backend_readiness(
    ...
    thread::spawn(move || {
        ...
        while process_started_at.elapsed() < deadline {
            if current_backend_pid(&app) != Some(pid) {
                return;
            }
            if current_backend_health(&app).as_deref() == Some("ready") {
                log_desktop_event(
                    &app,
                    &format!("event=health_monitor_completed pid={pid} source=already_ready"),
                );
                return;
            }
            attempt += 1;
            ...
            match probe_backend_health(&base_url, timeout) {
                Ok(()) => {
                    mark_backend_ready(&app, pid, process_started_at, "health_ready");
                    return;
                }
                Err(error) => {
                    last_error = Some(error.clone());
                    set_backend_health_starting(
                        &app,
                        &base_url,
                        pid,
                        process_started_at.elapsed().as_millis() as u64,
                        error,
                    );
        ...
        if let Ok(mut status) = state.status.lock() {
            if status.pid == Some(pid) && status.health != "ready" {
                status.health = "failed".to_string();
                status.diagnostic = "health_failed".to_string();
                status.uptime_ms = Some(process_started_at.elapsed().as_millis() as u64);
                status.last_health_error = Some(message.clone());
            }
        }
        log_desktop_event(
            &app,
            &format!("event=health_timeout pid={pid} base_url={base_url} error={message}"),
        );
        emit_backend_status_changed(&app);
    });
}

fn current_backend_pid(app: &AppHandle) -> Option<u32> {
    let state = app.state::<BackendSidecarState>();
    state.status.lock().ok().and_then(|status| status.pid)
}

fn current_backend_health(app: &AppHandle) -> Option<String> {
    let state = app.state::<BackendSidecarState>();
    state.status.lock().ok().map(|status| status.health.clone())
}

fn set_backend_health_starting(
    app: &AppHandle,
    ...
    if let Ok(mut status) = state.status.lock() {
        ...
        changed = status.last_health_error.as_deref() != Some(last_health_error.as_str());
        status.health = "starting".to_string();
        status.uptime_ms = Some(uptime_ms);
        status.last_health_error = Some(last_health_error);
    }
    if changed {
        emit_backend_status_changed(app);
    }
...
fn pick_backend_port() -> Result<(u16, bool), String> {
    if TcpListener::bind((BACKEND_HOST, 3001)).is_ok() {
        return Ok((3001, false));
    }

    for port in 3002..=3015 {
...
fn stop_backend_sidecar(app: &AppHandle) {
    let state = app.state::<BackendSidecarState>();
    log_desktop_event(app, "event=stop_requested");
    ...
    if let Ok(mut status) = state.status.lock() {
        ...
        if status.health != "failed" {
            status.health = "not_started".to_string();
            status.diagnostic = "not_started".to_string();
        }
    }
    log_desktop_event(app, "event=stop_completed");
    emit_backend_status_changed(app);
}
