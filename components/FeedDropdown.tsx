...
const FeedDropdown: React.FC<FeedDropdownProps> = ({
  ...
  const handleDelete = async (e: React.MouseEvent) => {
    e.stopPropagation();
    if (
      await confirmDanger(t("confirm.category_delete") + ` "${category.name}"?`)
    ) {
  ...
  const selectedClass = {
    ...
    centered: 'bg-white/12 text-white shadow-[0_0_16px_rgba(var(--color-primary),0.2)] border border-white/25 backdrop-blur-md',
    minimal: 'bg-transparent text-white border-white/40',
  }[variant];

  const unselectedClass = {
    default: 'text-gray-400 hover:text-white hover:bg-white/5 border border-transparent',
    centered: 'text-gray-300 hover:text-white hover:bg-white/6 border border-white/10',
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`${dotSizeClass} rounded-full transition-all duration-500 ${isSelected ? "shadow-[0_0_10px_currentColor] scale-125" : "opacity-70"}`}
        ...
        <HeaderIcons.ChevronDown
          size="xs"
          className={`transition-transform duration-500 ease-out ${isOpen ? "rotate-180 text-white" : "text-gray-500 group-hover:text-gray-300"} ${variant === 'minimal' ? 'opacity-0 group-hover:opacity-100' : ''}`}
        />
      </button>
                ...
                <div className="flex items-center space-x-1">
                  ...
                  {onEditCategory && (
                    <button
                      ...
                      className="p-2 rounded hover:bg-[rgb(var(--color-text))]/10 text-[rgb(var(--color-textSecondary))] min-w-[32px] min-h-[32px]"
