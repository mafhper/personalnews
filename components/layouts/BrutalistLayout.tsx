...
const BrutalistCard: React.FC<{ article: Article; index: number }> = ({ article, index }) => {
  ...
  return (
    <article 
      className={`group relative flex flex-col border-4 border-black dark:border-white bg-[rgb(var(--color-surface))] transition-all duration-300 ${
        isExpanded 
    ...
    >
      ...
      <div className={`flex items-center justify-between px-3 py-2 border-b-4 border-black dark:border-white text-[10px] font-bold uppercase tracking-wider ${
        isExpanded ? 'bg-[rgb(var(--color-accent))] text-white' : 'bg-black text-white dark:bg-white dark:text-black'
      }`}>
      ...
      {/* Media Area */}
      <div className={`relative w-full border-b-4 border-black dark:border-white group-hover:border-b-[rgb(var(--color-accent))] transition-colors ${
        isExpanded ? 'aspect-video h-full bg-black' : 'aspect-video'
      }`}>
        {isExpanded && embedUrl ? (
          <iframe
            src={embedUrl}
      ...
      <div className="p-4 flex flex-col flex-1 justify-between">
        <div>
          <h2 className={`font-bold leading-tight mb-3 uppercase group-hover:text-[rgb(var(--color-accent))] transition-colors ${
            isExpanded ? 'text-xl md:text-2xl' : 'text-lg line-clamp-3'
          }`}>
          ...
          <p className={`text-xs font-medium opacity-70 mb-4 border-l-2 border-current pl-2 ${
          ...
          <div className="flex gap-2 z-20">
            {embedUrl && (
              <button 
                ...
                className={`uppercase px-3 py-1 text-xs font-bold border-2 transition-all ${
                  isExpanded 
