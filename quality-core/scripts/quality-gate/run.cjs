 ...
 * 
 * Uso:
 *   node quality-core/scripts/quality-gate/run.cjs          # Todas as verificacoes
 *   node quality-core/scripts/quality-gate/run.cjs --quick  # Modo rapido (pula build)
 * 
 * Exit codes:
 *   0 = Aprovado
 *   1 = Falhou
...
async function checkI18n() {
    ...
    const i18nScript = path.join(config.paths.root, 'quality-core', 'scripts', 'i18n-audit.cjs');
    ...
    const result = await runCommand('node', ['quality-core/scripts/i18n-audit.cjs']);
...
async function checkSecrets() {
    ...
    const securityScript = path.join(config.paths.root, 'quality-core', 'scripts', 'security-scan.cjs');
    ...
    return {
        ...
        message: hasCritical ? 'Secrets expostos!' : (hasHigh ? 'Possiveis vazamentos' : 'OK'),
    };
}
...
async function main() {
    ...
    console.clear();
    UI.printHeader({
        title: 'QUALITY CORE - QUALITY GATE',
        modes: ['--quick'],
        active: isQuick ? ['quick'] : [],
    });
    ...
    const tasks = [
        { id: 'integrity', name: 'Integridade', run: checkIntegrity },
        { id: 'i18n', name: 'i18n', run: checkI18n },
        { id: 'security-audit', name: 'Segurança (npm audit)', run: checkSecurity },
        { id: 'secrets', name: 'Secrets Scan', run: checkSecrets },
        { id: 'lint', name: 'Lint', run: checkLint },
        {
            id: 'build',
            name: 'Build',
            run: checkBuild,
            skip: () => isQuick,
            skipMessage: 'Pulado (modo rapido)',
        },
        { id: 'performance', name: 'Performance', run: checkPerformance },
    ];

    UI.printPlan(
        tasks.map(task => ({
            name: task.name,
            avg: History.getAverageDuration(`quality-gate:${task.id}`),
        }))
    );
    ...
    for (let i = 0; i < tasks.length; i += 1) {
        ...
        if (task.skip && task.skip()) {
            result = { name: task.name, success: true, skipped: true, message: task.skipMessage || 'Pulado' };
        } else {
            result = await task.run();
        }

        const duration = result.duration || (Date.now() - taskStart);
        History.saveExecutionTime(`quality-gate:${task.id}`, duration);
        const avg = History.getAverageDuration(`quality-gate:${task.id}`);
        UI.printScriptEnd(task.name, duration, avg, result.success !== false);
