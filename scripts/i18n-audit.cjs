...
const CONFIG = {
    ...
    contextFile: path.resolve(__dirname, '../contexts/LanguageContext.tsx'),
    baseLocale: 'pt-BR',
    outputFile: path.resolve(__dirname, '../docs/logs/i18n-audit-report.md'),
};
...
function extractTranslations() {
    const content = fs.readFileSync(CONFIG.contextFile, 'utf8');
    const locales = {};
 ...
 */
function main() {
    console.log('Iniciando auditoria i18n...\n');

    // Extrair traducoes
    console.log('Extraindo traducoes...');
    const translations = extractTranslations();
    const locales = Object.keys(translations);
    console.log(`  Idiomas encontrados: ${locales.join(', ')}`);
    console.log(`  Chaves no ${CONFIG.baseLocale}: ${Object.keys(translations[CONFIG.baseLocale] || {}).length}`);

    // Verificar paridade
    console.log('\nVerificando paridade entre idiomas...');
    const parityIssues = checkTranslationParity(translations);
    console.log(`  Problemas encontrados: ${parityIssues.length}`);

    // Escanear strings hardcoded
    console.log('\nEscaneando strings hardcoded...');
    const hardcodedIssues = scanForHardcodedStrings(CONFIG.componentsDir);
    console.log(`  Strings suspeitas: ${hardcodedIssues.length}`);

    // Verificar chaves
    console.log('\nVerificando uso de chaves...');
    const keyIssues = scanForUndefinedKeys(CONFIG.componentsDir, translations);
    console.log(`  Problemas de chaves: ${keyIssues.length}`);
    ...
    console.log(`  Relatorio salvo em: ${CONFIG.outputFile}`);
    ...
    // Exit code
    if (totalIssues > 0 && process.argv.includes('--strict')) {
        process.exit(1);
    }
}
